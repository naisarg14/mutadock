...
def naisarg():
    ...
    #Clean the PDB file
    if not quiet: logger.info(f"Cleaning the PDB file {full_pdb_path}")
    cleaned_pdb = f"{full_pdb_path.removesuffix('.pdb')}_clean.pdb"
    with suppress_stdout():
        clean_pdb(full_pdb_path, cleaned_pdb)
    if not quiet: logger.info(f"PDB file cleaned successfully and saved as {cleaned_pdb}.")

    full_pdb_path = cleaned_pdb

    # Generates possible mutations
    if not quiet: logger.info(f"Generating mutations list for {full_pdb_path}")
    ...
    if file_info(out_all)[0] and file_info(out_op)[0] and append:
        ...
        mut_out = out_op
        if not quiet: logger.info(f"Files already exist, skipping generation of mutations list.")
    else:
        with suppress_stdout():
            mut_out, _ = generate_csv(full_pdb_path, out_all=out_all, out_op=out_op)
        if not quiet: logger.info(f"CSV for mutations generated successfully and saved as {mut_out}.")


    # Calculate ddG for the generated mutants
    if not quiet: logger.info(f"Calculating ddG values for for {mut_out}")
    out_file = f"{full_pdb_path.removesuffix('.csv')}_ddG.csv"
    if file_info(out_file)[0] and append and file_info(out_file)[1] == mut_length:
        ddg_out = out_file
        if not quiet: logger.info(f"File already exists, skipping calculation of ddG values.")
    else:
        with suppress_stdout():
            ddg_out = calc_ddg(full_pdb_path, mut_out, out_file=out_file)
        if not quiet: logger.info(f"ddG values for mutations calculated successfully and saved as {ddg_out}.")


    # Sorting the ddG values in descending order
    if not quiet: logger.info(f"Sorting the Single ddG values.")
    out_file = f"{ddg_out.removesuffix('.csv')}_sorted.csv"
    if file_info(out_file)[0] and append and file_info(out_file)[1] == mut_length:
        ddg_out_sort = out_file
        if not quiet: logger.info(f"File already exists, skipping sorting of ddG values.")
    else:
        with suppress_stdout():
            ddg_out_sort = sort_csv(ddg_out, out_file=out_file, col_num=8)
        if not quiet: logger.info(f"ddG values are sorted and stored as {ddg_out_sort}.")


    # Calculate Double ddG for the generated mutants
    if not quiet: logger.info(f"Calculating Double ddG values for {ddg_out_sort}.")
    ...
    if file_info(out_csv)[0] and append and file_info(out_csv)[1] == permutations(num_double_ddg, 2):
        ...
        if not quiet: logger.info(f"File already exists, skipping calculation of double ddG values.")
    else:
        with suppress_stdout():
            double_ddg_out = calc_double_ddg(full_pdb_path, out_csv=out_csv, single_csv=ddg_out_sort, total=num_double_ddg)
        if not quiet: logger.info(f"Double ddG values for mutations calculated successfully and saved as {double_ddg_out}.")


    # Sorting the ddG values in descending order
    if not quiet: logger.info(f"Sorting the Double ddG values.")
    ...
    if file_info(out_file)[0] and append and file_info(out_file)[1] == permutations(num_double_ddg, 2):
        ...
        if not quiet: logger.info(f"File already exists, skipping sorting of double ddG values.")
    else:
        with suppress_stdout():
            double_ddg_out_sort = sort_csv(double_ddg_out, out_file=out_file, col_num=11)
        if not quiet: logger.info(f"Double ddG values are sorted and stored as {double_ddg_out_sort}.")


    # Calculate Triple ddG for the generated mutants
    if not quiet: logger.info(f"Calculating Triple ddG values for {double_ddg_out_sort}")
    ...
    if file_info(out_csv)[0] and append and file_info(out_csv)[1] == permutations(num_triple_ddg, 3):
        ...
        if not quiet: logger.info(f"File already exists, skipping calculation of triple ddG values.")
    else:
        with suppress_stdout():
            triple_ddg_out = calc_triple_ddg(pdb_file=full_pdb_path, double_csv=double_ddg_out_sort, out_csv=None, total=num_triple_ddg)
        if not quiet: logger.info(f"Triple ddG values for mutations calculated successfully and saved as {triple_ddg_out}.")


    # Sorting the Triple ddG values in descending order
    if not quiet: logger.info(f"Sorting the Triple ddG values.")
    out_file = f"{triple_ddg_out.removesuffix('.csv')}_sorted.csv"
    if file_info(out_file)[0] and append and file_info(out_file)[1] == permutations(num_double_ddg, 2):
        double_ddg_out_sort = out_file
        if not quiet: logger.info(f"File already exists, skipping sorting of triple ddG values.")
    else:
        with suppress_stdout():
            triple_ddg_out_sort = sort_csv(triple_ddg_out, col_num=11)
        if not quiet: logger.info(f"Triple ddG values are sorted and stored as {triple_ddg_out_sort}")


    # Generate mutants and put them in a folder and csv with names for easy docking
    if not quiet: logger.info(f"Generating mutants for {ddg_out_sort}")
    ...
    if not quiet: logger.info(f"Generated single mutations PDB in {out_folder}")

    if not quiet: logger.info(f"Generating mutants for {double_ddg_out_sort}")
    with suppress_stdout():
    ...
    if not quiet: logger.info(f"Generated double mutations PDB in {out_folder}")

    if not quiet: logger.info(f"Generating mutants for {triple_ddg_out_sort}")
    with suppress_stdout():
    ...
    if not quiet: logger.info(f"Generated triple mutations PDB in {out_folder}")
