...
from .helpers import Mutation, backup, in_directory
...
def main() -> None:
    ...
    calc_double_ddg(
        pdb_file=full_pdb_path, single_csv=full_csv_path, total=total, out_csv=out_csv
    )
...
def calc_double_ddg(
    ...
    with open(out_csv, "w+") as out:
        ...
        for x in tqdm(combinations(title, 2)):
            ...
            pose_m1 = predict_ddG.mutate_residue(
                pose, pose_position1, mut1.new, 8.0, sfxn
            )
            pose_m2 = predict_ddG.mutate_residue(
                pose_m1, pose_position2, mut2.new, 8.0, sfxn
            )
